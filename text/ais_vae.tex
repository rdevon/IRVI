...
from variational inference by applying a series of transition operators
...
After we find $\tilde{q}$, we maximize the lower bound w.r.t. the model
parameters using stochastic gradient descent, using samples from the refined
approximate posterior. Our loss w.r.t. the global variational parameters becomes
...
where $\gamma$ is the learning rate and $(1 - \gamma)$ can be though of as the adaptive "damping" rate. Adaptive importance sampling, when applied iteratively, "recenters" the proposed distribution using a weighted average of samples, weighting by the suitability of each sample. This will clearly lead to a biased estimate of the lower bound when with a finite number of samples from $\tilde{q}$. However, we will show that it works very well in practice. This approach 

\section{Related Works}
Our work combines elements found in variational inference and MCMC methods. In
spirit, it is closest to the refinement procedure of hybrid MCMC for variational
inference \citep{salimans2014markov}, but uses a much simpler MCMC transition
than the Hamiltonian MC used there. In addition, the method detailed there
requires the same re-parameterization trick as in VAE, limiting it's application
to continuous latent variables.

Our work shares similarities with stochastic variational inference
...
Normalizing flows for VAE \citep{rezende2015variational} make use of auxiliary
networks to improve the approximate posterior. In doing so, they are able to
escape the factorial condition that GDIR and AdISIR have. However, they require
...
networks \citep[SFFN, ][]{tang2013learning}. Iterative refinement, however, uses
importance sampling to achieve a better posterior, using gradients of the lower
bound in optimization. 

\section{Experiments}

\subsection{Settings}

We test GDIR and AIR on MNIST, using the binarized dataset found in
\citep{salakhutdinov2008quantitative}, with a standard train, validation, and
test split of $50$k, $10$k, and $10$k samples. Unfortunately, recent papers on
...
the number of parameters and deterministic layers (if present) to match the
...
steps during inference when refining the approximate posterior.
...
validation \alert{waiting on some more results}. For comparison are
...
\citep[DLGM+NF,][]{rezende2015variational}, Hamiltonian MCMC variational
inference \citep[HVI,][]{salimans2014markov} with the number of leapfrog steps
specified, and importance weighted autoencoders
\citep[IWAE,][]{burda2015importance}. Included in table \ref{table:cont_step} is
...
Performance falls short of HVI with $10$ leapfrog steps, IWAE and DLGM+NF,
though not without notable advantages for those models.

We observe that the number of inference steps during learning improves lower
bound and log likelihood estimates. In addition, the bound becomes tighter both
as the number of inference during training and during evaluation.
...
\caption{Log likelihood and lower bound for a single continuous latent variable
...
\hline
<<<<<<< HEAD
$\ge$ & 96.24 & 93.72 & 93.36 & 91.89 & \\
\hline
$\approx$ & 95.24 & 91.59 & 90.85 & 89.51 & \\
\hline
\end{tabular}
...
For binary latent variables, we used two structures to compare to wake-sleep and
NVIL using both GDIR and AIR. For comparison to wake-sleep, we used a shallow
feed forward network with $200$ output neurons for the binary latent variables
for the posterior and no deterministic units for the posterior nor conditional.
As NVIL uses an additional feed forward network for baseline prediction, we used
an deterministic layer of $240$ softplus units in the recognition and generation
networks to roughly match the total number of parameters in each model. Models
were trained using an initial learning rate of \num{0.001} and $20$ samples
during inference and $20$ samples to approximate the lower bound during the
M-step.

GDIR and AIR models were trained with $1$, $5$, $10$, $20$, and $50$ steps. All
models were trained for $500$ epochs with the initial learning rate. For AIR, an
adaptive damping rate of $0.9$ was used, as lower damping rates proved unstable.
During test, $100$ steps were used during inference. Final models were selected
...
n & 1 & 5 & 10 & 20 & 50 \\
\hline
$\ge$ & 115.39 & 108.66 & 107.35 & 105.93 & 103.98 \\
\hline
$\approx$ & 109.77 & 106.9 & 104.86 & 103.49 & 101.67 \\
\hline
\end{tabular}
