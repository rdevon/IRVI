...
\citep{hinton1995wake}. Neural variational inference and learning (NVIL)
...
\begin{align}
\grad_{\TT^l} g(\TT^l, \epsilon) = \frac{1}{N} \sum_n \grad_{\TT^l} \big(\log p(x, h(\TT^l)^{(n)}) - \log \tilde{q}(h(\TT^l)^{(n)})\big).
\end{align}
...
Iterative refinement for variational inference (IRVI) combines elements found in variational inference and MCMC methods. In
spirit, it is closest to the refinement procedure of hybrid MCMC for variational
inference \citep[HVI,][]{icml2015_salimans15}, and in some senses HVI can be thought as a type of iterative refinement. However, the MCMC transitions in gradient descent iterative refinement (GDIR) and adaptive importance sampling iterative refinement (AIR) are substantially simpler than those used in HVI. In addition, HVI requires the same re-parameterization trick as in variational autoencoders (VAE), as is used in GDIR, limiting it's application to continuous latent variables.

GDIR also shares similarities with stochastic variational inference
\citep[SVI,][]{hoffman2013stochastic}, which also uses gradient descent to improve on the
variational inference algorithm. SVI, however, requires global latent variables with carefully chosen relationships with the local latent variables (our equivalent of $h$) to do inference.
...
Finally, IRVI is not entirely orthogonal to many variational inference methods, including many mentioned above, and could be combined to enhance results, particularly with variational inference methods that follow an EM training procedure.

\section{Experiments}

\subsection{Settings}

We test gradient descent iterative refinement (GDIR) and adaptive importance sampling iterative refinement (AIR) on the MNIST handwritten digit dataset, using the benchmark binarized dataset found in
...
coefficient of \num{0.0002}. During test, all models were evaluated using $100$
inference steps and $300$ inference samples during the E-step when refining the approximate posterior.
...
\citep[DLGM+NF,][]{rezende2015variational}, hybrid MCMC for variational
inference (HVI) with the number of leapfrog steps
specified, and importance-weighted autoencoders (IWAE). Included in Table \ref{table:cont_step} is
...
%\begin{minipage}{\textwidth}
    \begin{minipage}[l]{0.52\textwidth}
        \begin{tabular}{ | m{8em} | m{1.6cm} | m{1.6cm} | } 
            \hline
            Model & $\le$ -log p(x) & $\approx$ -log p(x) \\ 
            \hline
            \hline
            VAE \textasteriskcentered & 99 &  \\ 
            \hline
            \hline
            $GDIR_{20, 20}$ & 90.75 & 88.65 \\
            $GDIR_{20, 50}$ & 91.31 & 88.71 \\
            \hline
            \hline
            $HVI_5$\textdagger &  90.86 & 87.16 \\ 
            $HVI_{10}$\textdagger & 87.60 & 85.56 \\ 
            \hline
            DLGM+NF \textsection & 89.9 &\\
            \hline
            IWAE ($K=50$)\textdaggerdbl & & 84.78 \\ 
            \hline
        \end{tabular}
    \captionof{table}{Lower bounds and NLL for various continuous latent variable models and training algorithms. We use $200$ latent Gaussian variables. \textasteriskcentered{Approximate value from \cite{kingma2013auto} with $200$ latent variables.} \textdagger{Using the Larochelle dataset and validation dataset during training and $200$ latent variables}. \textsection{With multiple maxout layers and $40$ latent variables.} \textdaggerdbl{Sampling from continuous MNIST at each update with 2 deterministic layers with $200$ units and $50$ latent units.}}
    ...
    \begin{minipage}[r]{0.43\textwidth}
        ...
        model with varying number of inference steps during test.}
...
Hamiltonian MCMC with 5 leapfrog steps and DLGM+NF with diagonal covariance, despite conservative training.
Performance falls short of HVI with $10$ leapfrog steps and IWAE,
...
\begin{table}
    \centering
\begin{tabular}{ | m{1.6cm} | m{1cm} | m{1cm} | m{1cm} | m{1cm} | m{1cm} |}
\hline
n & 1 & 5 & 10 & 20 & 50 \\
\hline
$\le$ -log p(x) & 95.15 & 92.74 & 92.33 & 90.75 & 91.31\\
\hline
$\approx$ -log p(x) & 94.34 & 90.74 & 89.98 & 88.65 & 88.71 \\
\hline
\end{tabular}
\caption{Results for models trained with $n$ GDIR inference steps. Each model above was evaluated using $100$ steps and $300$ samples during inference. We report the lower bounds and estimated marginal probabilities on the test set.}
\label{table:cont_step}
\end{table}

\subsection{Discrete Variables}
Here we use Bernoulli variables, which have been widely used in previous related work. We used two different model parameterizations in order compare to results from wake-sleep, re-weighted wake-sleep (RWS), and neural variational inference and learning (NVIL) on 1-layer and 2-layer sigmoid belief networks (SBNs) against both GDIR (mu-push) and AIR. For comparison to wake-sleep and RWS, we used a shallow
...
\begin{table}[t]
    \begin{minipage}[l]{0.52\textwidth}
\begin{tabular}{ | m{8.7em} | m{1.6cm}| m{1.6cm} | } 
\hline
Model & $\le$ -log p(x) & $\approx$ -log p(x) \\ 
\hline
\hline
...
$AIR_{50}$ (200-200) & 96.62 & 92.26 \\
\hline
$GDIR_{20}$ & 129.94 & 113.6 \\
$GDIR_{20}$ (240 det sp) & 118.28  & 104.22 \\
\hline
\end{tabular}
\captionof{table}{Results for GDIR and AIR for a single-layer SBN with and without an intermediate softplus layer of $240$ units and a 2-layer SBN compared to similar models trained on the same MNIST dataset. All models have $100$ latent variables.}
\label{table:binary}
...
n & 1 & 5 & 10 & 20 & 50 \\
\hline
$\le$ -log p(x) & 110.92 & 108.46 & 107.76 & 106.72 & 104.52 \\
\hline
$\approx$ -log p(x) & 107.52 & 105.01 & 103.98 & 102.39 & 100.02 \\
\hline
\end{tabular}
\caption{Results for models without intermediate deterministic layer trained with AIR with $n$ inference steps. Each model above was evaluated using $200$ steps and $1000$ samples during inference.}
...
\section{Conclusion}
We have introduced iterative refinement for variational inference (IRVI), a unsophisticated approach for improving variational inference using a transition operator that maximizes the lower bound which is powerful and flexible. We achieve comparable performance to methods for improving VAE and state-of-the-art results with a single layer sigmoid belief network (SBN) with re-weighted wake-sleep (RWS) and 2-layer SBN RWS.
